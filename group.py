...
def calculate_average_similarity(group, similarity_matrix, files):
    ...
    for i, file in enumerate(indices):
        total_similarity = np.sum(similarity_matrix[file, indices])  # Calculates the sum of similarity to other files in the group
        count = len(indices) - 1  # exclude self
...
def group_files(hashes, threshold):
    ...
    for filename, hash_value in hashes.items():
        ...
        if not found_group:
            ...
            representatives[filename] = filename  # the representative file is itself
...
if __name__ == "__main__":
    ...
    similarity_threshold = 60  # similarity threshold，0-100
